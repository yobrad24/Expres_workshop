...
pokemon.post("/", async (req, res, netx) => {
    ...
    if (pok_name && pok_height && pok_weight && pok_base_experience) {
        ...
        if (rows.affectedRows == 1) {
            return res.status(201).json({ code: 201, message: "Pokemon instertado correctamente" });
        }
...
pokemon.delete("/:id([0-9]{1,3})",async(req,res,next)=>{
    ...
    if(row.affectedRows==1){
        return res.status(200).json({code: 20, message:"Pokemon borrado correctamente"});
    }
...
pokemon.get('/:id([0-9]{1,3})', async (req, res, next) => {
    ...
    return res.status(404).res.send({ code: 404, message: "Pokemon no encontrado" });
});
